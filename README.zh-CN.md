...
- **SDF Shadow Threshold Map**：对多张图的 SDF 进行插值生成平滑的阴影阈值图；支持 8/16 位位深、灰度/RGB/RGBA（带遮罩）色彩模式、高斯/双边滤波，并可反转渐变方向。
...
- **Tree Data Editor**（Design Tools）：节点式树状数据编辑器；支持节点拖拽、连线、框选、自动布局与网格吸附，可为节点配置属性键值，并支持 JSON 导入/导出与拖拽导入。
- **3D Dice**：基于 Three.js 与 cannon-es 的 3D 骰子模拟器；点击投掷骰子落入瓷碗，支持每次投掷 1/5/10/15/20 颗、实时计分与"决策板"高亮（总数对 6 取模），带 PCSS 软阴影、SSR/SSAO 后处理与碰撞音效。
